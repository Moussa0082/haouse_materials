    ...
    class User
    {
            ...
            else
            {
                if (!empty($pass) && !empty($rePass))
                {
                    if (strcmp(md5($pass), md5($rePass)) != 0)
                    {
                        echo "<div class='alert alert-danger alert-dismissable'>
                          <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
                          <strong>Warning !</strong> Mot de pass incorrect.
                      </div>";
                    }
                    else
                    {
                        ...
                        if ($mailCheck != false)
                        {
                            echo "<div class='alert alert-danger alert-dismissable'>
                                      <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
                                      <strong>Warning !</strong> Cet adresse e-mail existe déjà .
                                  </div>";
                        }
                        else
                        {
        ...
        public function loginUser($email, $pass)
        {
            ...
            else
            {
                ...
                else
                {
                    echo "<div class='alert alert-danger alert-dismissable'>
                          <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
                          <strong>Warning !</strong> Mot de passe or E-mail incorrect.
                      </div>";
                }
            }
        }
        ...
        public function updateUserInfoById($id, $data, $image)
        {
            ...
            if (empty($name) || empty($email) || empty($zip) || empty($phone) || empty($country) || empty($city) )
            {
                echo "<div class='alert alert-danger alert-dismissable'>
                          <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
                          <strong>Warning !</strong> Veuiller remplir ces champ.
                      </div>";
            }
            else
            {
